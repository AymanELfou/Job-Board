                        ...
                        <div x-data="{ applyModalOpen: false }" class="group bg-white border border-gray-100 rounded-[1.5rem] p-6 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 hover:border-[#a31b1b]/30 flex flex-col h-full relative">
                            ...
                            <div class="flex justify-between items-start mb-4">
                                ...
                                <form action="{{ route('jobs.save', $job->id) }}" method="POST" class="inline">
                                    @csrf
                                        <button type="submit" class="text-xl transition-transform hover:scale-110 {{ $job->isSavedBy(auth()->user()) ? 'text-[#a31b1b]' : 'text-gray-300 hover:text-gray-500' }}" title="{{ $job->isSavedBy(auth()->user()) ? 'Unsave' : 'Save' }}">
                                            <i class="bi {{ $job->isSavedBy(auth()->user()) ? 'bi-bookmark-fill' : 'bi-bookmark' }}"></i>
                                        </button>
                                    </form>
                                </div>
                                ...
                                <template x-teleport="body">
                                    <div x-show="applyModalOpen" style="display: none;" class="fixed inset-0 z-[100] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
