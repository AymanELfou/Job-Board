        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center py-1 px-4 gap-2">
            <div class="ml-2">
                <h2 class="text-4xl font-extrabold text-gray-900 tracking-tight">
                    Saved <span class="text-[#a31b1b]">Jobs</span>
                </h2>
                        ...
                        <div x-data="{ applyModalOpen: false }" class="group bg-white border border-gray-100 rounded-[1.5rem] p-6 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 hover:border-[#a31b1b]/30 flex flex-col h-full relative">
                            ...
                            <div class="flex justify-between items-start mb-4">
                                <span class="bg-blue-50 text-blue-600 px-3 py-1.5 rounded-xl text-xs font-bold uppercase tracking-wider border border-blue-100/50 shadow-sm flex items-center gap-1.5">
                                    <i class="bi bi-briefcase"></i> {{ $savedjob->job->job_type }}
                                </span>
                                
                                <form action="{{ route('jobs.save', $savedjob->job->id) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="text-xl transition-transform hover:scale-110 text-[#a31b1b]" title="Unsave">
                            ...
                            <!-- Content -->
                            <h4 class="text-xl font-bold text-gray-900 mb-3 leading-snug line-clamp-2">{{ $savedjob->job->titre }}</h4>
                            
                            <div class="space-y-2 mb-4">
                                <p class="text-gray-600 text-sm font-medium flex items-start gap-2">
                                    <i class="bi bi-building text-gray-400 mt-0.5"></i>
                                    {{ $savedjob->job->profilEmployer->nom_entreprise }} <span class="text-gray-300">•</span> {{ $savedjob->job->categorie }}
                                </p>
                                ...
                                <p class="text-gray-600 text-sm font-medium flex items-center gap-2">
                                    ...
                                    {{ $savedjob->job->salaire }} <span class="text-gray-300">•</span> {{ $savedjob->job->type_contrat }}
                                </p>
                            </div>
                            
                            <p class="text-gray-500 text-sm mb-6 line-clamp-3 leading-relaxed flex-grow">
                                {{ \Illuminate\Support\Str::limit($savedjob->job->description, 150, '...') }}
                            </p>
                            
                            <!-- Apply Button -->
                            <div class="mt-auto pt-4 border-t border-gray-100">
                                @if (!$savedjob->job->applications->where('id_jobseeker', auth()->user()->profile->id)->isEmpty())
                                        ...
                                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 border-b border-gray-100">
                                            <div class="flex justify-between items-center mb-5">
                                                <h3 class="text-2xl font-bold text-gray-900" id="modal-title">
                                                    Apply for <span class="text-[#a31b1b]">{{ $savedjob->job->titre }}</span>
                                                </h3>
                                            ...
                                            <form action="{{ route('jobs.apply', $savedjob->job->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                                                @csrf
                                                <div>
                                                    <label for="resume_{{ $savedjob->job->id }}" class="block text-sm font-bold text-gray-700 mb-2">Upload Resume (PDF, DOCX)</label>
                                                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-xl hover:border-[#a31b1b] transition-colors bg-gray-50">
                                                        <div class="space-y-1 text-center">
                                                            ...
                                                            <div class="flex text-sm text-gray-600 justify-center">
                                                                <label for="resume_{{ $savedjob->job->id }}" class="relative cursor-pointer bg-white rounded-md font-medium text-[#a31b1b] hover:text-[#8a1717] focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-[#a31b1b] px-1">
                                                                    <span>Upload a file</span>
                                                                    <input id="resume_{{ $savedjob->job->id }}" name="resume" type="file" class="sr-only" required>
                                                                </label>
                                                            </div>
                                                ...
                                                <div>
                                                    <label for="cover_letter_{{ $savedjob->job->id }}" class="block text-sm font-bold text-gray-700 mb-2">Cover Letter</label>
                                                    <textarea id="cover_letter_{{ $savedjob->job->id }}" name="cover_letter" rows="5" class="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-xl focus:ring-[#a31b1b] focus:border-[#a31b1b] p-3 transition-colors duration-200" placeholder="Introduce yourself and explain why you're a great fit for this role..." required></textarea>
                                                </div>
